...
function initLogin() {
  ...
  document.getElementById('forgot-password').addEventListener('click', () => {
    showToast('لأغراض العرض التجريبي، يرجى التواصل مع الدعم لاستعادة كلمة المرور', 'info', 3500);
  });
  ...
  document.getElementById('login-submit').addEventListener('click', async () => {
    ...
    // Newly-registered accounts must be reviewed by an admin before login.
    ...
    if (user.status === 'rejected') {
      ...
      const reasonPart = user.rejectionReason ? ` — السبب: ${user.rejectionReason}` : '';
      const supportPart = APP.supportPhone ? ` للاستفسار تواصل معنا: ${APP.supportPhone}` : '';
      showToast(`عذراً، تم رفض طلب التسجيل.${reasonPart}${supportPart}`, 'error', 6000);
      return;
    }
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    if (result.ok) {
      if (ctx.mode === 'register') {
        ...
        // Account awaits admin approval — do not sign the customer in.
...
function initWallet() {
  ...
  sheetConfirm.addEventListener('click', async () => {
    ...
    if (currentAction === 'deposit') {
      tx = addWalletTransaction({ type: 'deposit', title: 'إيداع رصيد', desc: 'إيداع عبر بطاقة إلكترونية', amount });
      showToast('تم إيداع الرصيد بنجاح', 'success');
    } else if (currentAction === 'withdraw') {
      tx = addWalletTransaction({ type: 'withdraw', title: 'سحب إلى حساب بنكي', desc: 'مصرف الرافدين •• 4821', amount: -amount });
      showToast('تم إرسال طلب السحب بنجاح', 'success');
    } else {
      tx = addWalletTransaction({ type: 'transfer_out', title: `تحويل إلى ${transferTarget}`, desc: 'تحويل بين المحافظ', amount: -amount });
