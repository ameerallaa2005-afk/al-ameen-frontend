...
function initSplash() {
  ...
  Promise.all([minDelay, AppState.sessionRestore]).then(() => {
...
// استعادة جلسة الدخول من التوكن المحفوظ (إذا موجود) قبل ما نقرر وجهة شاشة السبلاش،
// حتى ما يرجع المستخدم المسجَّل دخوله لشاشات onboarding/تسجيل الدخول من جديد.
if (Api.getToken()) {
  AppState.sessionRestore = Api.auth.me().then((res) => {
    if (res.ok) {
      AppState.currentUser = res.user;
      return syncAllFromServer().catch(() => {});
    } else if (!res.offline) {
