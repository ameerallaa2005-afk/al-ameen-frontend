      ...
      if (ctx.mode === 'register') {
        ...
        if (regResult.offline) {
          ...
          FAKE_USERS.push({
            ...
            name: ctx.name, city: ctx.city, status: 'active', verifiedBy: null, verifiedAt: null, rejectionReason: null,
          });
          AppState.currentUser = FAKE_USERS[FAKE_USERS.length - 1];
        } else if (regResult.ok && regResult.token) {
          // الحساب صار مفعّل مباشرة (رمز الواتساب هو التحقق الكافي) — نسجّل دخوله تلقائياً
          Api.setToken(regResult.token);
          AppState.currentUser = regResult.user;
        }

        AppState.pendingAuth = { mode: 'register', phone: ctx.phone, name: ctx.name, city: ctx.city };
...
document.addEventListener('DOMContentLoaded', () => {
  // استعادة جلسة الدخول من التوكن المحفوظ (إذا موجود) بشكل صامت بالخلفية
  if (Api.getToken()) {
    Api.auth.me().then((res) => {
      if (res.ok) {
        ...
        Api.setToken(null); // توكن منتهي أو غير صالح
      }
    }).catch(() => {});
  }
